...
function Huddles({ huddles, updateList, id, huddlesUserIsGoing }: Props) {
    ...
    return (
        <div className="flex flex-col justify-start px-2 h-full" id="carousel" >
            ...
            {huddles.map((huddle) => (
                <div
                    className="h-64 lg:h-72 p-2 flex-grow-1 3xl:pl-8 flex-shrink-0 shadow-md border-palette-dark hover:border-palette-orange bg-white bg-opacity-50 border relative rounded-lg"
                    key={huddle.id}
                >
                    {/* <HuddleCarouselItem */}
                    <NewHuddleCard
